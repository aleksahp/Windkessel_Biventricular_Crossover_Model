...
while ndone == 0 
    ...
    if sol.x(end) ~= tspan(end) 
        ...
        P_SA = sols(9,:) * 7.5; % Direct calculation instead, using Windkessel model.
...
V_SA = sols(:,9) * C_SA;
volumes.V_SA = (V_SA + V_SA_u) * 1e6;
volumes.V_SV = (sols(:,10) + V_SV_u) * 1e6; 
volumes.V_RV = sols(:,11) * 1e6; 
V_PA = sols(:,12) * C_PA;
volumes.V_PA = (V_PA + V_PA_u) * 1e6;
volumes.V_PV = (sols(:,13) + V_PV_u) * 1e6; 
V_SA_end = sols(end,9) * C_SA;
V_PA_end = sols(end,12) * C_PA;
volumes.Vtot = (sols(end,8) + V_SA_end + V_SA_u + sols(end,10) + V_SV_u + sols(end,11) + V_PA_end + V_PA_u + sols(end,13) + V_PV_u) * 1e6;
