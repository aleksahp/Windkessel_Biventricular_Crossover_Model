function [] = plot_exvivo_crossover_healthymodsev(loading,data)
...
Q_p_valve_mat_h = loading_h.Q_a_valve_mat; 
Q_p_valve_h = Q_p_valve_mat_h(:,base_loc_h);

P_PA_mat_h = loading_h.P_PA_mat;
P_PA_h = P_PA_mat_h(:,base_loc_h);
...
Q_p_valve_mat_m = loading_m.Q_a_valve_mat; 
Q_p_valve_m = Q_p_valve_mat_m(:,base_loc_m);

P_PA_mat_m = loading_m.P_PA_mat;
P_PA_m = P_PA_mat_m(:,base_loc_m);
...
Q_p_valve_mat_s = loading_s.Q_a_valve_mat; 
Q_p_valve_s = Q_p_valve_mat_s(:,base_loc_s);

P_PA_mat_s = loading_s.P_PA_mat;
P_PA_s = P_PA_mat_s(:,base_loc_s);
...
h2 = plot(V_RV_mat_m(:,base_loc_h), P_RV_mat_m(:,base_loc_h), 'g:',  'linewidth',2);
h3 = plot(V_RV_mat_s(:,base_loc_h), P_RV_mat_s(:,base_loc_h), 'r--', 'linewidth',2);
...
h2 = plot(V_RV_m, P_RV_m, 'g:',  'linewidth',2);
h3 = plot(V_RV_s, P_RV_s, 'r--', 'linewidth',2);
...
clf
hold on 
h1 = plot(P_PA_h, Q_p_valve_h, 'b',   'linewidth',2);
h2 = plot(P_PA_m, Q_p_valve_m, 'g:',  'linewidth',2);
h3 = plot(P_PA_s, Q_p_valve_s, 'r--', 'linewidth',2);

% title(strcat(run_experiment,'-RV'))

xlabel('Pulmonary Artery Pressure (mmHg)')
ylabel('Pulmonary Valve Flow (L/min)')
legend([h1 h2 h3],'H','M','S','location','Northwest')
...
%% Print figures
    ...
    if printoutfigs_on == 1
        ...
        print(hfig11, '-dpng',strcat('Figures/',run_experiment,'/Fk_Qp_PPA.png'))
        ...
        print(hfig11, '-depsc2',strcat('Figures/',run_experiment,'/Fk_Qp_PPA.eps'))        
