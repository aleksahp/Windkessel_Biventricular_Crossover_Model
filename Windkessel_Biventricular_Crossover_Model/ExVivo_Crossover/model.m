...
% Total stress (kPa)
crossover_scale_RV = 0.365; % Scale of crossover of RV function from LV.
crossover_scale_LV = 0.073; % Scale of crossover of LV function from RV.
sigma_LV  = (1 - crossover_scale_LV) * sigma_act_LV + crossover_scale_LV * sigma_act_RV  + sigma_pas_LV; 
sigma_SEP = sigma_act_SEP + sigma_pas_SEP; 
sigma_RV  = (1 - crossover_scale_RV) * sigma_act_RV + crossover_scale_RV * 0.5 * (sigma_act_LV + sigma_act_SEP)  + sigma_pas_RV;
...
%{
%% Previous Lumped circulatory model 
% Pressure (kPa)
P_SV = V_SV / C_SV; 
P_PV = V_PV / C_PV;  
...
%% Circulation Model Replacement
% Set pressures. (kPa)
P_SV = V_SV / C_SV; 
P_PV = V_PV / C_PV;  

% Valve flow diodes.
k = 20; % [1/P], makes change in P dimensionless. Sharpness of valve transitions between open and closed.
Z_SA = 0.05 * R_SA;
Z_PA = 0.05 * R_PA;

Q_m = max(((P_PV - P_LV) / R_m_vlv) * (1 / (1 + exp(-k * (P_PV - P_LV)))),0);
Q_a = max(((P_LV - P_SA) / (R_a_vlv + Z_SA)) * (1 / (1 + exp(-k * (P_LV - P_SA)))),0);
Q_t = max(((P_SV - P_RV) / R_t_vlv) * (1 / (1 + exp(-k * (P_SV - P_RV)))),0);
Q_p = max(((P_RV - P_PA) / (R_p_vlv + Z_PA)) * (1 / (1 + exp(-k * (P_RV - P_PA)))),0);
Q_SA = max(((P_SA - P_SV) / R_SA) * (1 / (1 + exp(-k * (P_SA - P_SV)))),0);
Q_PA = max(((P_PA - P_PV) / R_PA) * (1 / (1 + exp(-k * (P_PA - P_PV)))),0);
...
dxdt = [dxm_LV; dxm_SEP; dxm_RV; dym;           % 1-4
    ...
    dV_LV; dP_SA; dV_SV; dV_RV; dP_PA; dV_PV];   % 8-13
